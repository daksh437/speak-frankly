...
class ApiService {
  ...
  Future<TutorReply> sendChat({
    ...
    if (res.statusCode == 403) {
      ...
      // different things — the free daily cap, a hard paywall, or a PAYING
      ...
      // told their "free limit" was up and offered an upgrade they had already
      // bought.
  ...
  Future<DictionaryCard?> lookupWord(String word, {String? target}) async {
    // Encoded: the word is a path segment, and a phrase ("by the way") or an
    // accented form would otherwise build a malformed URL.
    final res = await _client
...
///
/// [code] is the server's own: `DAILY_LIMIT_REACHED` (free cap, ad reward can
/// lift it), `PREMIUM_REQUIRED` (hard paywall), or `FAIR_USE_LIMIT` — which
/// only a paying subscriber can ever see, and which must never be answered with
/// an upsell.
