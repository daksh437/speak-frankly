...
(async () => {
  ...
  try {
    ...
    // their daily MESSAGES run out — i.e. exactly when the session they
    // want a report for has just ended. The report is metered by the AUX
    // budget and must not be gated on the chat allowance as well.
    const chain = (path) => {
